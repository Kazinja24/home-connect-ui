...
const LandlordMessages = () => {
  ...
  const { data: leasesData, isLoading: leasesLoading } = useQuery({
    queryKey: ["landlord-leases-for-messages"],
      ...
      <div className="grid md:grid-cols-3 gap-4 h-[600px]">
        <Card className="glass-strong border-border/30">
          <CardContent className="p-4 space-y-2">
            ...
            {leasesLoading ? (
            ...
            ) : activeLeases.length > 0 ? activeLeases.map((l: any) => (
              <button
                key={l.id}
              ...
              >
                ...
                <p className="text-xs text-muted-foreground">{l.tenant_name || `${t("common.tenant")} #${l.tenant}`}</p>
              </button>
            )) : (
